...
for m = 1:M
    ...
    %Force the SR of the synthetic cores to equal the true average SR of
    %the set up by rescaling the ages
    t_coreAgesF{m} = t_coreAges{m}.*((t_coreDepths{m}(end)/t_aveSR(m))/(t_coreAges{m}(end)));
end
...
for m = 1:9
    subplot(3,3,m)
    plot(t_coreDepths{m}, t_coreAgesF{m}, 'k*')
    hold on
...
r_nSRs = cell(M,1);
for m = 1:M
    r_aveSR(m) = (t_coreDepths{m}(end)-t_coreDepths{m}(1))/(t_coreAgesF{m}(end) - t_coreAgesF{m}(1));
